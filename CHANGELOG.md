...
- **鍵の初回作成が並行すると、別の鍵が保存されて復号できなくなる問題**:
  ...
  「読む → 無ければ生成して保存」の形で、**直列化されていなかった**。同時に初回アクセスされると
  呼び出しごとに別々の鍵を作って後勝ちで保存されるため、**先に返した鍵で暗号化した列（ページ本文・
  付箋メモ）や pack が復号できなくなる**（CI では付箋メモが空として復元される flaky な失敗として
  現れた。取り込みは同時に走りうる）。生成〜保存をプロセス内で直列化し、ロックを待つ間に他の
  呼び出しが保存した値を拾うようにした。`crates/core/src/secrets.rs`、
  `crates/core/src/pack_keys.rs`

## [0.2.10] - 2026-09-27

> [!WARNING]
> **破壊的変更（データの互換性）**: DB の機密列（ページ本文・形態素解析・付箋メモ）を暗号化した
> 形式（`enc:v1:BASE64(IV||暗号文||タグ)`）で保存するようになった。起動時に既存の平文行を
> 一度だけ移行するので、**0.2.10 で起動した DB は 0.2.9 以前では正しく読めない**
> （復号できず、ページ本文・付箋メモが表示されない）。複数端末で使うときは、すべての端末を
> 0.2.10 以降に揃えてから同期すること。
