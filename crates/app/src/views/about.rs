...
const FIRST_STEPS: [(&str, &str); 3] = [
    ...
    (
        "3. 読む",
        "本をクリックすると取り込みが始まり、終わるとビューアーが開きます（取り込みには \
         Google ログインも必要です。本はアカウントごとの鍵で暗号化されます）。\
...
const SIDEBAR_ITEMS: [(&str, &str); 9] = [
    ...
    (
        "お気に入り",
        "ハートを付けた本。Google にログイン中のとき、同期時に未ダウンロードなら自動でダウンロードされます",
    ),
    (
        "閲覧履歴",
...
const DRIVE_NOTICE_TITLE: &str =
    "Google ログインは Google ドライブでのバックアップと本の取り込みに使用します";
const DRIVE_NOTICE_BODY: &str = "取り込んだ本を読むのに Google ログインは必要ありませんが、本の\
                                 取り込み（ダウンロード）には必要です（本は Google アカウントごとの\
                                 鍵で暗号化されるため、未ログインでは取り込みません）。Google ログインを\
                                 行うと、取り込んだ書籍ファイル本体（ページ画像を含むアプリの\
                                 形式に変換したもの）と本棚の DB を Google Drive にバックアップ・\
                                 同期できます。サイドバーのアカウントアイコンからログインしてください。";
...
const KEY_NOTICE_PASSPHRASE: &str = "パスフレーズを設定すると、Google に依存しない復元手段を\
                                     持てます（Google アカウントを使えない状況でも、Drive の\
                                     thundoku-keys.json とパスフレーズだけで鍵を戻せます）。\
                                     設定画面の一番上の「本の鍵」から設定・変更・解除ができます。\
                                     ログイン直後に未設定だと、このアプリが設定を促す案内を出します。\
                                     なお、thundoku-keys.json には「Google アカウントで解ける形」の\
                                     ラップも併存するため、Drive の中身とアカウント識別子の両方を\
                                     取得された相手は、パスフレーズが無くても鍵を解けます\
                                     （乗っ取りに対する追加防御にはなりません）。";
...
const FEATURES: [(&str, AppIcon, &str); 15] = [
    (
        "本棚",
        AppIcon::LibraryBig,
        "購入済み書籍をカードとリストの 2 表示で一覧表示。未読・読んでいる途中・読了のステータス、\
    ...
    (
        "チェックリスト",
        AppIcon::ListChecks,
        "イベントごとの購入チェック。**現在は技術書典専用**（他ストアのイベントには未対応）。技術書典と\
         同期、お気に入り登録、試し読み機能付き。",
    ),
    (
        "ダウンロードと取り込み",
        ...
        "各ストア（技術書典・BOOTH・FANZA・DLsite）からのダウンロード状況を一元管理。\
         完了・未完了・エラーの可視化と再試行ができます（取り込みには Google ログインが必要です。\
         本はアカウントごとの鍵で暗号化されます）。",
    ),
    (
        "Google Drive 同期",
    ...
    (
        ...
        "取り込んだ本は、アカウントごとの乱数で作られた鍵で暗号化されます。鍵は OS の資格情報\
         ...
         端末を失っても復元できます（Google アカウント侵害への追加防御にはなりません）。\
         設定画面の「本の鍵」から設定・変更・解除ができます。",
    ),
    (
        "関連書籍のショートカット",
        AppIcon::ArrowRight,
    ...
    (
        "対応ストア",
        AppIcon::Cloud,
        "各ストアの購入本を同期・取り込み。ストアごとにログインして本棚へ取り込めます。",
    ),
];
                                ...
                                div()
                                    ...
                                    .child(
                                        div()
                                            ...
                                            .line_height(relative(1.7))
                                            .child(
                                                "Thundoku Shelf は、技術書典（TechBookFest）などのオンラインストアで購入した同人誌・技術書をデスクトップで閲覧できるアプリです。購入履歴や書籍ファイルなどのデータはお使いの PC のローカルフォルダに保存されます（外部のサーバーには送りません。Google Drive バックアップを ON にしたときだけ、取り込んだ書籍ファイル本体と本棚の DB が Google Drive に保存されます）。",
                                            ),
                                    ),
                            ),
                    )
                                        ...
                                        div()
                                            ...
                                            .children(
                                                [
                                                    ...
                                                    "複数デバイス間で購入履歴と書籍ファイル本体を共有できます（Google Drive 同期）",
                                                    "同期は各画面の同期ボタンから開始します",
                                                ]
                                                .into_iter()
                                                .map(|item| {
...
mod tests {
    ...
    /// （鍵を失うと復号できない・パスフレーズは回復手段・v2 は取り込み直し）。
    #[test]
    fn about_explains_the_book_keys() {
        ...
        // パスフレーズは「Google に依存しない復元手段」であって、Drive アカウント侵害への
        // 追加防御にはならない（`sub` ラップが併存する）。ここを書かないと
        // 「乗っ取り時にも守られる」という誤った期待を持たせる（セキュリティ評価 F01）。
        assert!(
            KEY_NOTICE_PASSPHRASE.contains("追加防御にはなりません"),
            "パスフレーズの限界が説明画面から消えている: {KEY_NOTICE_PASSPHRASE}"
        );
        assert!(
