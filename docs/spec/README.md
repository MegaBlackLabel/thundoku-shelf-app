...
| 実装（対応中） | **大きな pack を RAM に載せない**: 閲覧（`PackFileReader`）・Drive からの取得（`download_to_file` でストリーム）・pack の組み立て（`build_to_file`）・ハッシュ計算・**Drive へのアップロード（`upload_multipart_from_file`）**は対応済み。取り込み中のページデータは `PackEntryStore` が保持し、**合計 64 MiB を超えたら一時ファイルへ逃がす**（pack の組み立ては 1 件ずつ供給 = ピークは「64 MiB + 1 ページ」）。PDF も `render_pdf_pages_into` で**1 ページずつ** store へ入れる（全ページを集めない）。**アプリのダウンロードも一時ファイル経由**（4 ストアの `download_to_file_with_progress` → パス経由の取込み。ピークは「1 ページ＋64 MiB」）。pack の上限は **10 GiB**（`MAX_TOTAL_SIZE` / `MAX_DOWNLOAD_BODY_BYTES`）。Drive へのアップロードは **resumable upload**（8 MiB ずつ + `308` で再開。2 GB ごとの分割は採らない。理由は `docs/spec/06-sync-auth-drive.md` §3.1）。**取り込み元は 10 GiB**（`MAX_IMPORT_SOURCE_BYTES` = pack と同じ）。PDF と ZIP は**ファイルから**読む（`import_pdf_path` / `import_zip_path`）ので、ソースの大きさがそのまま RAM にはならない。**残り**: アプリのダウンロードがまだメモリ経由（`download_with_progress`）なので、4 GiB 級は一時的に大きめの RAM を使う（一時ファイル経由へ切り替える）。2 GiB 超の本はダウンロード前に確認ダイアログを出す（`crates/core/src/store_size.rs`） |
...
| 運用（継続） | **公開はテスト・監査の合格に依存させる**（セキュリティ評価 F07）: `release.yml` の `checks` ジョブが `ci.yml` を再利用可能ワークフローとして呼び、`build` → `release` が `needs` で連なる。同じコミットでテストと `cargo audit` が通らなければ**ビルドも公開もしない**。Actions は全て完全なコミット SHA で固定（`upload-artifact` の例外も解消）。依存の勧告はコードを変えなくても増えるため、**週次の定期実行**（`ci.yml` の `schedule`）で `test` と `audit` を回す（60 日間リポジトリに動きが無いと GitHub が定期実行を止めるので、`gh workflow list` で `active` を時々確認する） |
