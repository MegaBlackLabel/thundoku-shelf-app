...
- オンラインストアで**購入した同人誌・技術書（PDF / 画像 ZIP / EPUB、非DRM のみ）**を、デスクトップで管理・閲覧するアプリ。
...
| **読書状態** | 未読 / 読書中 / 読了 の 3 値 | 判定は 1 箇所（`ReadingState::from_progress`）に集約 |
...
- [ ] **読書状態の 3 値統一**（未読 / 読書中 / 読了）と全画面での一貫表示
...
| 箇所 | 内容 |
|---|---|
| `docs/features.md` の一部 | 検索対象の記述が古い（実装は著者も対象）、Drive の ON/OFF の説明が実装とずれる |
| `docs/account-switch.md` | Drive フォルダをアカウント別キーと記載（実装は単一キー） |
| `docs/database.md` | `view_history::touch` をアプリが使うように読める（実際はテストのみ）、`scroll_position` は常に 0 |
| コード内コメント | 通知の自動消滅を「3 秒」と書いた箇所がある（実装は gpui-kit の既定 **5 秒**） |
| コード内コメント | `crates/core/src/import/pdf.rs` に「target width of 800px」と書かれた箇所がある（実装は **1000px**） |
| 説明画面 | かつて「リーダー」「技術書典限定のダウンロード」等の古い記述があった（更新済み。`docs/spec/04-ui.md` に現行文言） |

詳細と根拠は `docs/spec/07-decisions.md` の「未対応・既知の制約」を参照。
