...
ない。`sub` ラップを消す「パスフレーズ必須モード」は §9 の将来オプション。

| 漏れたもの | `sub` ラップのみ | パスフレーズラップあり（現行＝`sub` ラップ併存） | `sub` ラップ無し（将来の必須モード） |
|---|---|---|---|
...
- **失効は限定的**: パスフレーズを変えても**`sub` ラップが残っている限り `sub` を知る相手は解ける**
  ...
  （§9 の「パスフレーズ必須モード」）にしたときだけ、`sub` を知る相手を締め出せる。
...
- **パスフレーズ必須モード**（`sub` ラップを消す＝Google ログインだけでは解けなくする）。
  現行はパスフレーズを設定しても `sub` ラップが残るため、**Drive 侵害への追加防御にはならない**
  （§1.2）。導入する場合の代償: 新端末で Google ログインだけでは読めなくなり（Web 版の
  「ログインだけで読める」経路も止まる）、パスフレーズ入力が必須になる。
  既定にするかどうかの設定化は、この mode を入れたうえでの論点。
