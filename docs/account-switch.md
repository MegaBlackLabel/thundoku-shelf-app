...
|---|---|---|---|---|
| `NULL`（未所属） | **ログイン中でない**時の DL。ただし**現在の取り込みは Google ログインが必須**（未ログインでは平文 pack を作らず失敗させる = fail-closed。セキュリティ評価 F03）なので、**新規には作られない**（旧版で取り込んだ本が残る） | **ログアウト中のみ表示** | そのときだけ読める（未暗号化） | ❌ しない |
| `= 現在 sub` | そのアカウントでログイン中の DL | 表示 | 読める | ✅ する |
| `≠ 現在 sub` | 他のアカウントで DL | 非表示 | 読めない（キーが無い） | ❌ しない |

## ログイン・切替・ログアウト

- **DL の挙動**：ログイン中（sub S）だと `owner_sub = S` で暗号化。**取り込みには Google ログインが
  必須**（未ログインでは平文 pack を作らず失敗させ、ダウンロードを始める前にログインを促す =
  fail-closed。セキュリティ評価 F03）なので、**新規に `owner_sub = NULL`（未暗号化）の本は作られない**。
  既存の NULL 行はそのまま保有する。
